...

{
    ...
    private static string CachePath => Path.Combine(AgentHubConfig.Dir, "litellm.prices.cache.json");
    ...
    /// <summary>测试/探针注入点：记录/替代 LiteLLM 刷新入口；生产不设置。</summary>
    internal static Func<bool, Task>? RefreshOverrideForTest;

    /// <summary>测试/探针注入点：用固定 JSON 加载 LiteLLM 价，避免访问网络。</summary>
    internal static bool LoadRatesFromJsonForTest(string json)
    {
        if (!TryParse(json, out var rates) || rates.Count == 0) return false;
        lock (Gate)
        {
            _rates = rates;
            _source = "test";
    ...
    /// <summary>测试/探针：清空 LiteLLM 价视图与刷新注入点；生产不调用。</summary>
...

    {
        ...
        if (RefreshOverrideForTest is { } over)
        {
            await over(force).ConfigureAwait(false);
            return;
        }
    ...
    /// <summary>去掉 cn: / qoder/ 等短厂商·区域前缀；无前缀返回 null。</summary>
...

    {
...

        {
...

            {
                ...
                return last >= 0 && last < rest.Length - 1
    ...
    /// <summary>仅精确键整行回退；不经过旧缓存比率映射（避免 Fast 取到普通价），也不剥 -fast/-high 后缀。</summary>
    public static bool TryGetExactPrice(string? model, out PriceRow row)
    {
        row = null!;
        var name = (model ?? "").Trim();
        ...
        return TryMakeRow(rates, name, name, out row) && row.InputPer1m is { } and > 0;
    }

    /// <summary>快照式精确查价：用捕获的只读价视图，不读全局 _rates。</summary>
    public static bool TryGetExactPrice(LiteLlmPrices prices, string? model, out PriceRow row)
    {
        row = null!;
        var name = (model ?? "").Trim();
        if (name.Length == 0) return false;
        if (!prices.TryGet(name, out var lite)) return false;
        row = ToPriceRow(name, lite);
        return row.InputPer1m is { } and > 0;
    }

    /// <summary>捕获当前 LiteLLM 价为只读视图，供一次用量查询内整行回退使用。</summary>
    public static LiteLlmPrices CapturePrices()
    {
        Dictionary<string, LiteRates>? rates;
        lock (Gate) rates = _rates;
        var copy = new Dictionary<string, LiteLlmRate>(StringComparer.OrdinalIgnoreCase);
        if (rates is not null)
        {
            foreach (var (key, value) in rates)
            {
                if (!double.IsFinite(value.InputPer1m) || !double.IsFinite(value.OutputPer1m)) continue;
                copy[key] = new LiteLlmRate(value.InputPer1m, value.OutputPer1m, value.CacheReadPer1m, value.CacheWritePer1m);
            }
        }
        return new LiteLlmPrices(copy);
    }

    private static PriceRow ToPriceRow(string modelName, LiteLlmRate lite) => new()
    {
        Model = modelName,
...

    {
        ...
        // ZCode 等：cn:deepseek-v4-flash → deepseek-v4-flash



        var bare = StripRegionPrefix(name);
...

        {
            ...
            if (TryResolveKey(rates, bare, out row)) return true;



            if (PriceAliases.TryMap(bare, out aliased) && TryResolveKey(rates, aliased, out row))
            ...
            name = bare; // 后续剥 Cursor 后缀也基于无前缀名
