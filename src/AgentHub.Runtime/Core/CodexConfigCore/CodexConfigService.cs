...

{
...

    {
        ...
        keySet = !string.IsNullOrEmpty(Secrets.Unprotect(c.ApiKeyCipher)),
...

    {
        ...
        if (plainKey.Length > 0) conn.ApiKeyCipher = Secrets.Protect(plainKey);
...

    {
        ...
        return Secrets.Unprotect(conn.ApiKeyCipher);
    ...
    /// 清空当前 ~/.codex/auth.json（先备份），让下次打开 Codex 进入未登录、可重新 ChatGPT 登录。

    /// 不改 config.toml；若当前是 API Key，清掉后即可走官方订阅登录流。

    /// </summary>

    public async Task<CodexApplyResult> ClearLiveAuthAsync()

    {
...

        {
...

            {
                ...
                backupPath = AuthJsonPath + ".agenthub-bak-" + DateTime.Now.ToString("yyyyMMdd-HHmmss");

                File.Copy(AuthJsonPath, backupPath, overwrite: false);

                File.Delete(AuthJsonPath);
...

            {
                ...
                RestartRequired = IsCodexRunning(),

                BackupPath = backupPath,

                Note = backupPath is null

                    ? "当前已无 auth.json，打开 Codex 会要求登录"

                    : "已清空登录（备份在 " + backupPath + "）。请彻底退出并重启 Codex，再按提示用 ChatGPT 登录；登录后可回 AgentHub「导入当前」。",
...

        {
            ...
            return new CodexApplyResult { Ok = false, Error = "清空登录失败：" + ex.Message };
