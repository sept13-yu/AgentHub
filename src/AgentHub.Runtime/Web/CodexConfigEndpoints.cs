...

{
...

    {
        ...
        app.MapPost("/api/codex-config/auth-profiles/clear-live", async (HttpContext ctx) =>

        {
...

            {
                ...
                var result = await service.ClearLiveAuthAsync();
        ...
        app.MapPost("/api/codex-config/official/prepare-blank", async (HttpContext ctx) =>
        {
            if (!writeAuth(ctx))
                return Results.Json(new { error = "forbidden：写操作仅限 AgentHub 壳内" }, statusCode: 403);
            try
            {
                var result = await service.PrepareOfficialBlankAsync();
                return Results.Json(result);
            }
            catch (Exception ex)
            {
