...
async function clearLiveAuth() {
  clearAuthShow.value = false
  if (readonly || clearingAuth.value) return
  clearingAuth.value = true
  if (pageLoading) pageLoading.value = true
  try {
    const r = await post<{ ok: boolean; restartRequired?: boolean; note?: string; error?: string }>(
      '/api/codex-config/auth-profiles/clear-live', {})
    if (!r.ok) {
      message.error(r.error || '清空失败')
      return
    }
    if (r.restartRequired) message.warning((r.note || '已清空登录') + '（检测到 Codex 在运行，请彻底退出后重启）')
    else message.success(r.note || '已清空登录。打开 Codex 会要求重新登录')
    await loadProfiles()
    await load()
  } catch (e) {
...
<template>
    ...
    <section class="card profile-card">
      <div class="card-head">
        ...
        <n-button
          ...
          :disabled="readonly || clearingAuth"
          @click="clearAuthShow = true"
        >
          清空登录
        </n-button>
        <n-button
  ...
  <ConfirmDialog
    :show="clearAuthShow"
    title="清空当前登录？"
    text="将备份并删除 ~/.codex/auth.json。打开 Codex 后会要求重新用 ChatGPT 登录；登好后再回这里「导入当前」归档。中转 API Key 不会写进档案。"
    confirm-text="清空登录"
    danger
